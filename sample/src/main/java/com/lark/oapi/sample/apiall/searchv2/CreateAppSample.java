package com.lark.oapi.sample.apiall.searchv2;

import com.lark.oapi.Client;
import com.lark.oapi.core.utils.Jsons;
import com.lark.oapi.service.search.v2.model.CreateAppReq;
import com.lark.oapi.service.search.v2.model.CreateAppReqBody;
import com.lark.oapi.service.search.v2.model.CreateAppResp;

// POST /open-apis/search/v2/app
public class CreateAppSample {

    public static void main(String arg[]) throws Exception {
        // 构建client
        Client client = Client.newBuilder("appId", "appSecret").build();

        // 创建请求对象
        CreateAppReq req = CreateAppReq.newBuilder()
                .userIdType("user_id")
                .pageSize(20)
                .pageToken("")
                .createAppReqBody(CreateAppReqBody.newBuilder()
                        .query("测试应用")
                        .build())
                .build();

        // 发起请求
        CreateAppResp resp = client.search().app().create(req);

        // 处理服务端错误
        if (!resp.success()) {
            System.out.println(String.format("code:%s,msg:%s,reqId:%s"
                    , resp.getCode(), resp.getMsg(), resp.getRequestId()));
            return;
        }

        // 业务数据处理
        System.out.println(Jsons.DEFAULT.toJson(resp.getData()));
    }
}
